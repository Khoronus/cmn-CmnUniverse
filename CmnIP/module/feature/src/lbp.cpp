...
#else
	std::cout << "TODO: Fix debug error caused by missing var (channel) " <<
...
cv::Mat LBP::OLBP(const cv::Mat& src) { cv::Mat dst; OLBP(src, dst); return dst; }
cv::Mat LBP::ELBP(const cv::Mat& src, int radius, int neighbors) { cv::Mat dst; ELBP(src, dst, radius, neighbors); return dst; }
cv::Mat LBP::VARLBP(const cv::Mat& src, int radius, int neighbors) { cv::Mat dst; VARLBP(src, dst, radius, neighbors); return dst; }
