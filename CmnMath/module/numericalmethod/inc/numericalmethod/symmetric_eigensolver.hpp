...
#ifndef CMNMATH_NUMERICALMETHOD_SYMMETRICEIGENSOLVER_HPP__
#define CMNMATH_NUMERICALMETHOD_SYMMETRICEIGENSOLVER_HPP__
...
#include "cmnmathcore\inc\cmnmathcore\range_iteration.hpp"
...
const
{
    if (0 <= c && c < mSize)
    {
        ...
        for (auto const& givens : core::reverse(mGivens))
        {
        ...
        {
            size_t numBytes = mSize*sizeof(Real);
            core::Memcpy(eigenvector, x, numBytes);
        }
    }
}
//----------------------------------------------------------------------------
