...
#ifndef CMNMATH_NUMERICALMETHOD_MINIMIZEN_HPP__
#define CMNMATH_NUMERICALMETHOD_MINIMIZEN_HPP__

#include <cstring>
#include "cmnmathcore\inc\cmnmathcore\wrapper.hpp"
#include "algebra\inc\algebra\gvector.hpp"
#include "minimize1.hpp"
...
private:
    ...
    int mMaxIterations;
	std::vector<algebra::GVector<Real>> mDirections;
    int mDConjIndex;
    int mDCurrIndex;
    algebra::GVector<Real> mTCurr;
	algebra::GVector<Real> mTSave;
    Real mFCurr;
    ...
    Real const* tInitial, Real* tMin, Real& fMin)
{
    ...
    core::Memcpy(&mTSave[0], tInitial, numBytes);
	core::Memcpy(&mTCurr[0], tInitial, numBytes);
	...
	core::Memcpy(tMin, &mTCurr[0], numBytes);
    fMin = mFCurr;
}
//----------------------------------------------------------------------------
template <typename Real>
