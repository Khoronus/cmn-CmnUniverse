...
#ifndef CMNMATH_NUMERICALMETHOD_LINEARSYSTEM_HPP__
#define CMNMATH_NUMERICALMETHOD_LINEARSYSTEM_HPP__

#include <map>
#include "cmnmathcore\inc\cmnmathcore\wrapper.hpp"
#include "algebra\inc\algebra\matrix2x2.hpp"
#include "algebra\inc\algebra\matrix3x3.hpp"
#include "algebra\inc\algebra\matrix4x4.hpp"
...
public:
	...
	static bool Solve(algebra::Matrix2x2<Real> const& A, algebra::Vector2<Real> const& B,
		algebra::Vector2<Real>& X);

	static bool Solve(algebra::Matrix3x3<Real> const& A, algebra::Vector3<Real> const& B,
		algebra::Vector3<Real>& X);

	static bool Solve(algebra::Matrix4x4<Real> const& A, algebra::Vector4<Real> const& B,
		algebra::Vector4<Real>& X);
...
//----------------------------------------------------------------------------
template <typename Real>
bool LinearSystem<Real>::Solve(algebra::Matrix2x2<Real> const& A,
	algebra::Vector2<Real> const& B, algebra::Vector2<Real>& X)
{
    bool invertible;
	algebra::Matrix2x2<Real> invA = algebra::Inverse(A, &invertible);
    if (invertible)
    {
        X = invA * B;
    }
    ...
    {
		X = algebra::Vector2<Real>::Zero();
    }
    return invertible;
}
//----------------------------------------------------------------------------
template <typename Real>
bool LinearSystem<Real>::Solve(algebra::Matrix3x3<Real> const& A,
	algebra::Vector3<Real> const& B, algebra::Vector3<Real>& X)
{
    bool invertible;
	algebra::Matrix3x3<Real> invA = algebra::Inverse(A, &invertible);
    if (invertible)
    {
        X = invA * B;
    }
    ...
    {
		X = algebra::Vector3<Real>::Zero();
    }
    return invertible;
}
//----------------------------------------------------------------------------
template <typename Real>
bool LinearSystem<Real>::Solve(algebra::Matrix4x4<Real> const& A,
	algebra::Vector4<Real> const& B, algebra::Vector4<Real>& X)
{
    bool invertible;
	algebra::Matrix4x4<Real> invA = algebra::Inverse(A, &invertible);
    if (invertible)
    {
        X = invA * B;
    }
