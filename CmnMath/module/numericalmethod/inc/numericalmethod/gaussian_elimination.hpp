...
#ifndef CMNMATH_NUMERICALMETHOD_GAUSSIANELIMINATION_HPP__
#define CMNMATH_NUMERICALMETHOD_GAUSSIANELIMINATION_HPP__
    ...
    int numCols, Real* Y) const
{
    ...
    core::Array2<true, Real> matInvM(numRows, numRows, inverseM);
	core::Array2<true, Real> matY(numRows, numCols, Y);
#else
	core::Array2<false, Real> matInvM(numRows, numRows, inverseM);
	core::Array2<false, Real> matY(numRows, numCols, Y);
#endif
    ...
    Real* target) const
{
    if (std::is_floating_point<Real>() == std::true_type())
    {
        ...
        if (source)
        {
            core::Memcpy(target, source, numBytes);
        }
        else
        {
